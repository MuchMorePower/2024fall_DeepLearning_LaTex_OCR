...
def main():
    ...
    # 设置优化器，分别为每层的权重设置不同的 L2 正则化强度
    optimizer = torch.optim.Adam(
        [
            {'params': encoder_params, 'lr': 0.001, 'weight_decay': 1e-4},  # 编码器的学习率和正则化
            {'params': decoder_params, 'lr': 0.0005, 'weight_decay': 1e-4},  # 解码器的学习率和正则化
        ],
        betas=(0.9, 0.999),
        eps=1e-8
    )
    ...
    for epoch in range(num_epochs):
        ...
        with tqdm(enumerate(train_loader), total=len(train_loader)) as it:
            for i, (imgs, caps, caplens) in it:
                ...
                #targets 的seq_len为 predictions.size(1) - 1
                #形状为(batchsize,decode_length) 去掉了起始的<start>
                targets = caps_sorted[:, 1:]

                #print("decoder length: ",decode_lengths)
                #print(predictions)
...
def validateTest(val_loader, model, criterion, index_word_map, epoch):
    ...
    with torch.no_grad():  # Disable gradient computation
        ...
        with open("lengths_output.txt", "w") as f:
            ...
            if len(references) < 10 or len(hypotheses) < 10:
                print("Not enough data to randomly select five samples.")
            else:
            # 随机选择5个不同的索引
                random_indices = random.sample(range(len(references)), 10)
